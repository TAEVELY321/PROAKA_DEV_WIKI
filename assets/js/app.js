(function () {
  ...
  function allSearchText(character) {
    return [character.name, character.nameEn, character.school, character.club, character.major, character.role, character.position, character.summary, ...character.tags, ...character.protocols, ...character.skills.flatMap((skill) => [skill.name, skill.type, skill.trigger])].join(' ').toLocaleLowerCase('ko');
  }
  ...
  function renderCharacter() {
    ...
    rootEl.innerHTML = `<header class="article-header character-title"><div><span class="eyebrow">PLAYABLE CHARACTER · ${esc(character.rarity)}</span><h1>${esc(character.name)}</h1><p class="lead">${esc(character.nameEn)}</p></div><div class="article-meta">${statusBadge('TBD')} 개별 사양 검수 필요</div></header>
      ...
      <div class="character-layout"><div class="character-copy">
        ...
        <section class="article-section" id="overview"><h2><span>1.</span> 개요</h2>${character.overview.map((paragraph) => `<p>${esc(paragraph)}</p>`).join('')}<div class="source-note"><b>기획 기준</b><span>캐릭터별 기존 전투 키트는 v2.3에서 보존되지만, 개별 프로필·수치는 최신 통합 기획서에서 재확인되지 않아 TBD로 구분합니다.</span></div></section>
        <section class="article-section" id="profile"><h2><span>2.</span> 프로필</h2><div class="profile-facts extended-profile"><div><span>학교</span><b>${esc(character.school)}</b></div><div><span>전공</span><b>${esc(character.major)}</b></div><div><span>역할</span><b>${esc(character.role)}</b></div><div><span>전용무기</span><b>${esc(character.weapon)}</b></div><div><span>생일</span><b>${dataValue(character.internal.birthday)}</b></div><div><span>신장</span><b>${dataValue(character.internal.height)}</b></div><div><span>쓰리사이즈</span><b>${dataValue(character.internal.measurements)}</b></div><div><span>핵심 컬러</span><b>${dataValue(character.coreColor)}</b></div></div><h3>컬러 팔레트</h3>${paletteMarkup(character)}<p class="field-help">임시 팔레트입니다. 다섯 색 모두 회색으로 지정되어 있으며 최종 아트 승인 시 <code>palette</code> 값을 교체합니다.</p></section>
        <section class="article-section" id="personality"><h2><span>3.</span> 인물 소개</h2><h3>외형</h3><p>${esc(character.appearance)}</p><h3>성격</h3><ul>${character.personality.map((item) => `<li>${esc(item)}</li>`).join('')}</ul></section>
        <section class="article-section" id="history"><h2><span>4.</span> 작중 행적</h2>${character.history.map((item) => `<h3>${esc(item.title)}</h3><p>${esc(item.body)}</p>`).join('')}</section>
        <section class="article-section" id="combat"><h2><span>5.</span> 인게임 정보</h2><div class="combat-summary"><span>${esc(character.position)}</span><p>${esc(character.summary)}</p></div><div class="notice notice-compact"><strong>v2.3 전투 원칙</strong><span>패시브가 반복 규칙을 만들고 기본공격이 자원을 축적·소비하며 전투스킬이 변환·폭발의 결론을 만듭니다. 무기는 장기 운용 분기, 모듈은 개별 조건·대상·결과를 조정합니다.</span></div><div class="skill-list">${character.skills.map((skill) => `<article class="skill-card"><div class="skill-type">${esc(skill.type)}</div><div><h3>${esc(skill.name)}</h3><span class="skill-trigger">발동 · ${esc(skill.trigger)}</span><p>${esc(skill.body)}</p><ul>${skill.effects.map((effect) => `<li>${esc(effect)}</li>`).join('')}</ul></div></article>`).join('')}</div><h3>추천 속성·전술 프로토콜 ${statusBadge(character.protocolStatus)}</h3><div class="synergy-row">${character.protocols.map((item) => `<span>${esc(item)}</span>`).join('')}</div><p class="field-help">학교·동아리는 탐험 사건, 시설 보너스, 관계 묘사의 문맥으로 사용하며 강한 글로벌 전투 시너지에는 사용하지 않습니다.</p></section>
      ...
      </div><aside class="character-infobox" style="--character-color:${esc(character.color)}"><div class="infobox-name"><strong>${esc(character.name)}</strong><span>${esc(character.nameEn)}</span></div><div class="portrait-frame">${imageOrPlaceholder(character, 'detail-portrait')}</div><dl class="compact-info"><div><dt>성별</dt><dd>${esc(character.gender)}</dd></div><div><dt>종족</dt><dd>${esc(character.species)}</dd></div><div><dt>학교</dt><dd>${esc(character.school)}</dd></div><div><dt>동아리</dt><dd>${esc(character.club)}</dd></div><div><dt>전공</dt><dd>${esc(character.major)}</dd></div><div><dt>포지션</dt><dd>${esc(character.position)}</dd></div><div><dt>무기 타입</dt><dd>${esc(character.weaponType)}</dd></div><div><dt>전용무기</dt><dd>${esc(character.weapon)}</dd></div><div><dt>핵심 컬러</dt><dd>${dataValue(character.coreColor)}</dd></div><div class="palette-info-row"><dt>컬러 팔레트</dt><dd>${paletteMarkup(character, true)}</dd></div><div><dt>신장</dt><dd>${dataValue(character.internal.height)}</dd></div><div><dt>쓰리사이즈</dt><dd>${dataValue(character.internal.measurements)}</dd></div><div><dt>생일</dt><dd>${dataValue(character.internal.birthday)}</dd></div></dl></aside></div>`;
  }
